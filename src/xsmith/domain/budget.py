...
class Budget(BaseModel):
    """Tracks remaining execution count, tokens, and dollars.

    The exploration loop's primary termination condition is `exec_remaining`:
    each call to the runner counts as one execution. Tokens and USD are
    *recorded* for telemetry but don't gate iteration unless `enforce_cost`
    is True.
    """
